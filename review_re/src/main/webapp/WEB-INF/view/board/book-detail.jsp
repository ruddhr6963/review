...
<div class="container">
	...
	<div class="align-right">
		<c:choose>
		    <c:when test="${mode eq 'write'}">
		    	<input type="button" class="btn" onclick="location.href='${pageContext.request.contextPath}/book-write?item=${board.item_no }'" value="글쓰기">	        
		    </c:when>
		    <c:when test="${mode eq 'preview'}">
		...
		<input type="button" class="btn" value="목록보기" onclick="location.href='${pageContext.request.contextPath}/list?item=${item }';"/>	
	</div>
	<script>
		...
		$('#revise').on("click", function(){
			 var $form = $('<form></form>');
		     $form.attr('action', '${pageContext.request.contextPath}/book-revise?no=${board.no}&item_no=${board.item_no}');
		     $form.attr('method', 'post');
		     $form.appendTo('body');
		...
		$('#delete').on("click", function(){
			var $form = $('<form></form>');
			$form.attr('action', '${pageContext.request.contextPath}/book-delete/${board.no }/${board.item_no }');
			$form.attr('method', 'post');
			$form.appendTo('body');
		...
		function form_hidden($form){
			//book
		     ...
		     var ir1 = $('<input type="hidden" value="${board.detail}" name="ir1">');
		     
		     $form.append(image).append(book_title).append(author).append(publisher).append(pubdate).
		     				append(item_no).append(head).append(writer).append(title).append(ir1);
	     	 $form.submit();
		}
	</script>
